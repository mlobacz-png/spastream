...
export async function generateTreatmentPlanPDF(plan: TreatmentPlan) {
  ...
  pdf.setFontSize(7);
  pdf.text(
    `Generated on ${format(new Date(), 'MMMM dd, yyyy')} | MedSpaFlow`,
    pageWidth / 2,
    finalY + 4,
    { align: 'center' }
  );
...
function addPrivacyPolicyPage(pdf: jsPDF) {
  ...
  const sections = [
    ...
    },
    {
      ...
      content: 'MedSpaFlow maintains strict HIPAA compliance standards. All PHI is encrypted both in transit (TLS 1.3) and at rest (AES-256). Access to PHI is restricted through authentication and role-based access controls.'
    },
    {
      title: 'Data Security',
    ...
    },
    {
      ...
      content: 'Please store this document securely and dispose of it properly when no longer needed. Do not share this document with unauthorized individuals. If you have questions about our privacy practices, contact: privacy@medspaflow.com'
    }
  ];
  ...
  pdf.setTextColor(29, 78, 216);
  pdf.setFont('helvetica', 'normal');
  pdf.text('www.medspaflow.com/privacy  |  www.medspaflow.com/terms', margin + 5, yPos + 16);
}
