...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Extract the project reference from the Supabase URL and construct the app URL
    const projectRef = supabaseUrl.replace('https://', '').split('.supabase.co')[0];
    const paymentUrl = `https://${projectRef}.supabase.co/pay/${paymentLink.unique_token}`;
